...
public class ATM {	
	...
	{
		Scanner scan = new Scanner(System.in);
		System.out.print("계좌번호를 입력하시오 : (6자리)");
	...
	{
		Scanner scan = new Scanner(System.in);
		System.out.print("이체할 계좌번호 입력하시오 : (6자리)");
	...
	{
		Scanner scan = new Scanner(System.in);
		System.out.print("도난신고할 계좌 입력 : (6자리)");
		String account = scan.next();
		return account;
	}
	...
	{
		Scanner scan = new Scanner(System.in);
		System.out.println("Password : (4자리)");
		String pw = scan.next();
		return pw;
	}
		...
		if(admin.CheckAdminID(id))
		{
			...
			if(admin.CheckAdminPW(pw))
			{
				...
				else if(index == 2)
				{
					...
					if(amount < 0)
					{
						System.out.println("Cannot enter minue value!");
					}
					else
					{
						admin.AdminCalculate(amount);
